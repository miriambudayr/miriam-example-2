package com.configure_me_miriam_example_2.api.core
...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString().orElseThrow { MiriamExample2InvalidDataException("Value is not a string") }
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw MiriamExample2InvalidDataException("`$name` is not set")
            is JsonNull -> throw MiriamExample2InvalidDataException("`$name` is null")
            else -> throw MiriamExample2InvalidDataException("`$name` is invalid, received $this")
        }
